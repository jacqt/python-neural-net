...
if __name__ == "__main__":
    ...
    testImage = Image.open(testFile).convert("L")
    imgArr = numpy.asarray(testImage)
    k = map (lambda row: map (lambda x: 255-x*100,row), LabelPixels(imgArr))
    nlist = numpy.array(k, dtype=numpy.uint8)
    im= Image.fromarray(nlist,"L")
    im.save("f.png","png")

    j = numpy.array(LabelPixels(imgArr),dtype=numpy.uint8)
    PasteCharacters(imgArr,j)
